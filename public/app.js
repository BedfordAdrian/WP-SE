...
views.books = async () => {
  const books = await api.get('/books');
  main.innerHTML = pageHead('Books', 'Your catalogue — the raw material for every post.',
    `<button class="btn primary" id="b-new">＋ Add book</button>`)
    + (books.length ? `<div class="grid cards">${books.map(bookCard).join('')}</div>`
      : `<div class="empty"><div class="big">📚</div>No books yet. Add one to start generating content.</div>`);
...
function bookFormModal(existing) {
  ...
  openModal(`<h2>${existing ? 'Edit' : 'Add'} book</h2>
    ...
    <div class="form-row">
      <div class="field"><label>Tropes (comma-separated)</label><input id="bf-tropes" value="${esc((b.tropes || []).join(', '))}"></div>
    ...
    <div class="field"><label>Amazon / buy link</label><input id="bf-buy" value="${esc((b.buyLinks && (b.buyLinks.universal || b.buyLinks.amazon)) || '')}"></div>
  ...
  $('#bf-save').addEventListener('click', async () => {
    const body = {
      ...
      tropes: $('#bf-tropes').value.split(',').map((s) => s.trim()).filter(Boolean),
